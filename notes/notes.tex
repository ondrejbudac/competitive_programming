...
You play a game with $N \leq 15$ bosses. 
At the beginning you have a weapon that takes $1$ life per shot from any boss.
For every boss $i \in \{0, \ldots, N-1\}$ you know that his gun takes $a_{ij}$ life per shot from boss $0 \in \{ 1, \ldots, N-1\}$.
When life points of a boss drop to $0$ or less, you defeated him and you have (and can use) his weapon.
...
You are given a path of a robot as a string containing only characters $L$ (turn $60$ degrees left), $R$ (turn $60$ degrees right), and $F$ (move $1$ meter forward).
A second robot start in the exact position (and rotation) as the first robot.
Its goal is to get to the final point of the first robot by going only on his tracks (no matter which direction).
For the second robot it takes $3$ seconds to turn left or right by $60$ degrees.
Going forward one meter takes $4$ seconds and two meters take $8$ seconds, but for $n>2$, the second robot can go $n$ meters forward (without turning) in $8+2(n-2)$ seconds.
Calculate the minimum time needed for the second robot to get to the destination.
\nextpart
...
At position $p_i \in \mathbb{Z}$ on a line we have $c_i$ cats for $0 \leq i \leq N \leq 1000$. 
Every minute, each cat that sitting at the point $p$ has three choices: stay at $p$, move to $p+1$, move to $p-1$.
Return minimum possible number of points where more than one cat sits after $T$ minutes.
\nextpart
...
\begin{questionanswer}{codeforces round 265, division 1, C}{modulo, dp}{cf:265:1:B}
todo
\nextpart
\insertcode{"codes/codeforces265div1C.cpp"}{todo}
\end{questionanswer}

\begin{questionanswer}{codeforces round 266, division 1, D}{modulo, dp}{cf:266:1:D}
todo
\nextpart
