...
%\insertcode{"codes/EdmondKarp.cpp"}{A C++ code for the Edmond-Karp algorithm for maximal flow.} 
...
\item if that edge connects two different trees, then add it to the forest, combining two trees into a single tree
\end{itemize}
\end{itemize}
...
Determine for every step the number of cells that change color from white to black.
 ...
 \chapter{Strings}
string matching, suffixes, prefixed, \ldots
 ...
 Note that $Z[i] = 0$ means tat $S[0] \neq S[i]$.
 
 \insertcode{"codes/Zalgorithm.cpp"}{todo}
 
 A standard application of the Z alorithm is for the string matching.
...
\begin{questionanswer}{topcoder SRM 633, division 1, easy}{greedy, triangle inequality}{srm:633:1:easy}
Given is a point $(x,0)$, where $|x| \leq 10^9$ and $x \in \mathbb{N}$.
A frog starts at the point $(0,0)$ and it wants to get to the point $(x,0)$ using as few jumps as possible.
You are given a sequence of positive integers $d_0, d_1, \ldots, d_{N-1}$, where $1 \leq N \leq 50$.
The frog can jump in any direction but her $i$-th jump (starting from $i=0$) must have length $d_{i \% N}$.
\nextpart
\insertcode{"codes/PeriodicJumping.cpp"}{todo}
\end{questionanswer}
